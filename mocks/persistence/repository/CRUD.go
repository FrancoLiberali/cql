...
type CRUD[T model.Model, ID model.ID] struct {
	mock.Mock
}

// Create provides a mock function with given fields: tx, entity
func (_m *CRUD[T, ID]) Create(tx *gorm.DB, entity *T) error {
...
func (_m *CRUD[T, ID]) Delete(tx *gorm.DB, entity *T) error {
	ret := _m.Called(tx, entity)
...
func (_m *CRUD[T, ID]) Find(tx *gorm.DB, conditions ...orm.Condition[T]) ([]*T, error) {
	...
	var r0 []*T
	var r1 error
	if rf, ok := ret.Get(0).(func(*gorm.DB, ...orm.Condition[T]) ([]*T, error)); ok {
		return rf(tx, conditions...)
	}
	if rf, ok := ret.Get(0).(func(*gorm.DB, ...orm.Condition[T]) []*T); ok {
		r0 = rf(tx, conditions...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*T)
	...
	if rf, ok := ret.Get(1).(func(*gorm.DB, ...orm.Condition[T]) error); ok {
		r1 = rf(tx, conditions...)
	} else {
		r1 = ret.Error(1)
	}
...
func (_m *CRUD[T, ID]) FindOne(tx *gorm.DB, conditions ...orm.Condition[T]) (*T, error) {
	_va := make([]interface{}, len(conditions))
	for _i := range conditions {
		_va[_i] = conditions[_i]
	}
...
func (_m *CRUD[T, ID]) GetByID(tx *gorm.DB, id ID) (*T, error) {
	ret := _m.Called(tx, id)
...
func (_m *CRUD[T, ID]) Save(tx *gorm.DB, entity *T) error {
	ret := _m.Called(tx, entity)
...
type mockConstructorTestingTNewCRUD interface {
	mock.TestingT
	Cleanup(func())
}
...
func NewCRUD[T model.Model, ID model.ID](t mockConstructorTestingTNewCRUD) *CRUD[T, ID] {
	mock := &CRUD[T, ID]{}
	mock.Mock.Test(t)
