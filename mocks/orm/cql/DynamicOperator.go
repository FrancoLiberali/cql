...
func (_m *DynamicOperator[T]) SelectJoin(valueNumber uint, joinNumber uint) cql.DynamicOperator[T] {
	ret := _m.Called(valueNumber, joinNumber)

	var r0 cql.DynamicOperator[T]
	if rf, ok := ret.Get(0).(func(uint, uint) cql.DynamicOperator[T]); ok {
		r0 = rf(valueNumber, joinNumber)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(cql.DynamicOperator[T])
		}
	}
...
func (_m *DynamicOperator[T]) ToSQL(query *cql.GormQuery, columnName string) (string, []interface{}, error) {
	ret := _m.Called(query, columnName)

	var r0 string
	var r1 []interface{}
	var r2 error
	if rf, ok := ret.Get(0).(func(*cql.GormQuery, string) (string, []interface{}, error)); ok {
		return rf(query, columnName)
	}
	if rf, ok := ret.Get(0).(func(*cql.GormQuery, string) string); ok {
		r0 = rf(query, columnName)
	} else {
		r0 = ret.Get(0).(string)
	}

	if rf, ok := ret.Get(1).(func(*cql.GormQuery, string) []interface{}); ok {
	...
	if rf, ok := ret.Get(2).(func(*cql.GormQuery, string) error); ok {
