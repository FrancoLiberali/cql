...
func (ts *OperatorsIntTestSuite) TestEqPointers() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.IntPointer.Is().Eq(cql.Float32(1)),
	).Find()
...
func (ts *OperatorsIntTestSuite) TestEqNullableType() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.NullFloat.Is().Eq(cql.Float64(1.3)),
	).Find()
...
func (ts *OperatorsIntTestSuite) TestNotEq() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().NotEq(cql.Int(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestLt() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Lt(cql.Int(3)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestLtOrEq() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().LtOrEq(cql.Int(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestGt() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Gt(cql.Int(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestGtOrEq() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().GtOrEq(cql.Int(3)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestBetween() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Between(cql.Int(3), cql.Int(5)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestNotBetween() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().NotBetween(cql.Int(0), cql.Int(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestIsDistinct() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Distinct(cql.Int(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestIsNotDistinct() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().NotDistinct(cql.Int(3)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestArrayIn() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.String.Is().In(cql.String("s1"), cql.String("s2"), cql.String("s3")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestArrayNotIn() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.String.Is().NotIn(cql.String("ns1"), cql.String("ns2")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericTypeWithSameType() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value()),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericTypeWithDifferentType() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.Float.Value()),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForCustomType() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.MultiString.Is().Eq(conditions.Product.MultiString.Value()),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForBaseModelAttribute() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.CreatedAt.Is().Eq(conditions.Product.CreatedAt.Value()),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNotNullTypeCanBeComparedWithNullableType() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Float.Is().Eq(conditions.Product.NullFloat.Value()),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestUnsafeOperatorInCaseTypesNotMatchConvertible() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Float.IsUnsafe().Eq(cql.String("2.1")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestUnsafeOperatorInCaseTypesNotMatchNotConvertible() {
	...
	case cqlSQL.SQLite:
		...
		entities, err := cql.Query[models.Product](
			ts.db,
			conditions.Product.Float.IsUnsafe().Eq(cql.String("not_convertible_to_float")),
		).Find()
		ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithPlus() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Plus(1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithMinus() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Minus(1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithTimes() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Times(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithDivided() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Divided(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithModulo() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Modulo(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithPower() {
	...
	case cqlSQL.SQLite:
		_, err := cql.Query[models.Product](
			ts.db,
			conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Power(2)),
		).Find()
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithSquareRoot() {
	...
	case cqlSQL.SQLite:
		_, err := cql.Query[models.Product](
			ts.db,
			conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().SquareRoot()),
		).Find()
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithAbsolute() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Absolute()),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithAnd() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().And(1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithOr() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Or(3)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithXor() {
	...
	case cqlSQL.SQLite:
		_, err := cql.Query[models.Product](
			ts.db,
			conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Xor(3)),
		).Find()
		ts.ErrorIs(err, cql.ErrUnsupportedByDatabase)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithNot() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Not().And(5)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithShiftLeft() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().ShiftLeft(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithShiftRight() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().ShiftRight(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithMultipleFunction() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Plus(1).Times(2)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForNumericWithFunctionOfDifferentType() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.Int.Is().Eq(conditions.Product.IntPointer.Value().Times(1.5)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *OperatorsIntTestSuite) TestDynamicOperatorForStringWithConcat() {
	...
	entities, err := cql.Query[models.Product](
		ts.db,
		conditions.Product.String.Is().Eq(conditions.Product.String2.Value().Concat("123")),
	).Find()
	ts.Require().NoError(err)
