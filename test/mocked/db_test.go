...
func TestWithLoggerFromContext(t *testing.T) {
	...
	db, err := cql.Open(postgres.New(postgres.Config{
		Conn: conn,
	}), cql.WithLoggerFromContext(func(context.Context) logger.Interface {
	...
	_, err = cql.Insert(context.Background(), db, &user).Exec()
	require.NoError(t, err)
	...
	assert.Contains(t, log, "test/mocked/db_test.go:48")
