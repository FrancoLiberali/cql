...
func (ts *WhereConditionsIntTestSuite) TestQueryReturnsEmptyIfNotEntitiesCreated() {
	entities, err := cql.Query[models.Product](context.Background(), ts.db).Find()
...
func (ts *WhereConditionsIntTestSuite) TestConditionsReturnsEmptyIfNotEntitiesCreated() {
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.String.Is().Eq(cql.String("not_created")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionsReturnsEmptyIfNothingMatch() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.String.Is().Eq(cql.String("not_match")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfFloatType() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.Float.Is().Eq(cql.Float64(1.1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfID() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.ID.Is().Eq(cql.UUID(match.ID)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfCreatedAt() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.CreatedAt.Is().Eq(cql.Time(match.CreatedAt)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfDeletedAt() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.DeletedAt.Is().Eq(cql.Time(match.DeletedAt.Time)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfEmbedded() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.ToBeEmbeddedEmbeddedInt.Is().Eq(cql.Int(1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfGormEmbedded() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.GormEmbeddedInt.Is().Eq(cql.Int(1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfPointerTypeWithValue() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.IntPointer.Is().Eq(cql.Int(1)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfByteArrayWithContent() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.ByteArray.Is().Eq(cql.ByteArray([]byte{1, 2})),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfByteArrayEmpty() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		conditions.Product.ByteArray.Is().Eq(cql.ByteArray([]byte{})),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfRelationType() {
	...
	entities, err := cql.Query[models.Sale](
		context.Background(),
		ts.db,
		conditions.Sale.ProductID.Is().Eq(cql.UUID(product1.ID)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionOfRelationTypeOptionalWithValue() {
	...
	entities, err := cql.Query[models.Sale](
		context.Background(),
		ts.db,
		conditions.Sale.SellerID.Is().Eq(cql.UUID(seller1.ID)),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestConditionsOnUIntModel() {
	...
	entities, err := cql.Query[models.Brand](
		context.Background(),
		ts.db,
		conditions.Brand.Name.Is().Eq(cql.String("match")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestMultipleConditionsAreConnectedByAnd() {
	...
	entities, err := cql.Query[models.Product](
		...
		conditions.Product.Int.Is().LtOrEq(cql.Int(4)),
		conditions.Product.String.Is().Eq(cql.String("match")),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestNot() {
	...
	entities, err := cql.Query[models.Product](
		context.Background(),
		ts.db,
		cql.Not(conditions.Product.Int.Is().Eq(cql.Int(2))),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestNotWithMultipleConditionsAreConnectedByAnd() {
	...
	entities, err := cql.Query[models.Product](
		...
		ts.db,
		cql.Not(
			conditions.Product.Int.Is().Gt(cql.Int(1)),
			conditions.Product.Int.Is().Lt(cql.Int(4)),
		),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestOr() {
	...
	entities, err := cql.Query[models.Product](
		...
		cql.Or(
			...
			conditions.Product.String.Is().Eq(cql.String("match_3")),
		),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestNotOr() {
	...
	entities, err := cql.Query[models.Product](
		...
		cql.Not[models.Product](
			cql.Or(
				conditions.Product.Int.Is().Eq(cql.Int(2)),
				conditions.Product.String.Is().Eq(cql.String("not_match_string")),
			),
		),
	).Find()
	ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestXor() {
	...
	case sql.MySQL:
		...
		entities, err := cql.Query[models.Product](
			...
			ts.db,
			mysql.Xor(
				conditions.Product.Int.Is().Lt(cql.Int(6)),
				conditions.Product.Int.Is().Gt(cql.Int(3)),
			),
		).Find()
		ts.Require().NoError(err)
...
func (ts *WhereConditionsIntTestSuite) TestMultipleConditionsDifferentOperators() {
	...
	entities, err := cql.Query[models.Product](
		...
		ts.db,
		conditions.Product.String.Is().Eq(cql.String("match")),
		conditions.Product.Int.Is().Lt(cql.Int(2)),
		conditions.Product.Bool.Is().True(),
	).Find()
	ts.Require().NoError(err)
