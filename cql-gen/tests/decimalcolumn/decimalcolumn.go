...
var errUnsupportedScan = errors.New("unsupported scan source type")
...
func (m *Money) Scan(src any) error {
	v, ok := src.(int64)
	if !ok {
		return fmt.Errorf("%w %T for Money", errUnsupportedScan, src)
	}
...
func (n *Note) Scan(src any) error {
	v, ok := src.(string)
	if !ok {
		return fmt.Errorf("%w %T for Note", errUnsupportedScan, src)
	}
