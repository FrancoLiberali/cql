...
func testSetNotNecessaryWithFunction() {
	cql.Update[models.Phone](
		...
		conditions.Phone.Name.Set().Eq(conditions.Phone.Name.Appearance(0).Concat(cql.String("asd"))), // want "Appearance call not necessary, github.com/FrancoLiberali/cql/test/models.Phone appears only once"
	)
}
...
func testSetNecessaryNotCalledWithFunction() {
	cql.Update[models.Child](
		...
		conditions.Child.Name.Set().Eq(conditions.ParentParent.Name.Concat(cql.String("asd"))), // want "github.com/FrancoLiberali/cql/test/models.ParentParent appears more than once, select which one you want to use with Appearance"
	)
}
